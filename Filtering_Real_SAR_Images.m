...
for looklp=1:2
 ...
 %The following homogeneous regions are slelected to caculate the corresponding ENLs 
  ...
  %% Results
    ...
    ENL=cal_enl(Y1,Y2,X1,X2,img,FANS_filtered,FoE_Nakafiltered,proposed);
    subplot(2,4,1);imshow(img,[]);title(['noisy image',',ENL:',num2str(ENL(1))]);hold on
    rectangle('Position',[X1,Y1,X2-X1+1,Y2-Y1+1],'EdgeColor','w','Linewidth',1);
    subplot(2,4,2);imshow(FANS_filtered,[ ]);title(['FANS,ENL:',num2str(ENL(2))]);
    subplot(2,4,3);imshow(FoE_Nakafiltered,[ ]);title(['Naka-FoE,ENL:',num2str(ENL(3))]);
    subplot(2,4,4);imshow(proposed,[ ]);title(['Proposed,ENL:',num2str(ENL(4))]);

    Ratio=img./(img);
    Ratio_FANS_filtered=img./(FANS_filtered+0.1);
    Ratio_FoE_Nakafiltered=img./(FoE_Nakafiltered+0.1);
